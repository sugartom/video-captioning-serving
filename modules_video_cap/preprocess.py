...
def build_vocab(word_count_thresh):
    ...
    sents_train = open(TEXT_DIR + '/sents_train_lc_nopunc.txt','r').read().splitlines()
    sents_val = open(TEXT_DIR + '/sents_val_lc_nopunc.txt','r').read().splitlines()
    sents_test = open(TEXT_DIR + '/sents_test_lc_nopunc.txt','r').read().splitlines()
    unk_required = False
    all_captions = []
    word_counts = {}
