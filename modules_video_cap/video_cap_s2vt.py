...
class S2VT:
  ...
  def Apply(self):
    ...
    if self.input['features'] is not None:
      ...
      np.save("feature_original.npy", self.input['features'])
      ...
      # self.log("vid type " + str(vid.dtype))
      # self.log("caption type " + str(self.caps.dtype))
      # self.log("caption mask type " + str(caps_mask.dtype))
      # self.log("dropout type " + str(self.dropout.dtype))
      print(self.caps)
      print(caps_mask)
      print(self.dropout)
      ...
      for i in range(self.n_steps):
        o_l = self.sess.run(self.output_logits,feed_dict={self.video:vid,
        ...
        if id2word[output_captions[0][i]] == '<EOS>':
          break
      np.save("logits_original.npy", np.array(outlogits_list))
