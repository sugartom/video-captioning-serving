...
class CapAlexnet:
  ...
  def PreProcess(self, request, istub, features_fc7, my_lock, grpc_flag):
    ...
    self.image = cv2.resize(self.image.astype(np.float32), (self.image_size, self.image_size))
    self.image -= CapAlexnet.imagenet_mean
    self.image = self.image.reshape((1, self.image_size, self.image_size, 3))
  ...
  def Apply(self):
    ...
    if (len(self.features_fc7) >= CapAlexnet.out_dims):
